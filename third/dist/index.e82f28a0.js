...
toggler.addEventListener('click', (e)=>{
    isRunning = !isRunning;
    !isRunning || activeTime === 0 ? (clearInterval(timerInterval), e.target.textContent = !activeTime ? 'RESTART' : 'START') : (e.target.textContent = 'PAUSE', timerInterval = setInterval(()=>{
        (!isRunning || activeTime === 0) && (clearInterval(timerInterval), e.target.textContent = !activeTime ? 'RESTART' : 'START');
        isRunning && (activeTime -= 1000);
        assignTime(activeTime);
    }, 1000));
});
settingsIcon.addEventListener('click', modalToggler);
...
colorsWrapper.addEventListener('click', (e)=>{
    const target = e.target;
    if (target.classList.contains("bg-red") || target.classList.contains("bg-teal") || target.classList.contains("bg-purple")) {
        colorLabels.forEach((colorLabel)=>{
            if (target !== colorLabel) colorLabel.textContent = '';
        });
        target.textContent = '✔';
    }
...
function assignTime(time) {
    minutesTag.textContent = addZero(Math.floor(time / 60000 % 60));
...
function changeSettings(arrow, input, oldInputValue, mode) {
    ...
    arrow.addEventListener('click', (e)=>{
        e.target.classList.contains('upward-arrow') ? newValue += 1 : newValue -= 1;
    ...
    closeIcon.addEventListener('click', ()=>{
        ...
        fontLabels.forEach((fontLabel)=>{
            if (fontLabel.classList.contains('bg-black') || fontLabel.children[0].value !== activeFont) {
                fontLabel.classList.remove('bg-black', 'text-gray-light');
                fontLabel.classList.add('bg-gray');
            } else if (fontLabel.children[0].value === activeFont && fontLabel.classList.contains('bg-gray')) {
                fontLabel.classList.remove('bg-gray');
                fontLabel.classList.add('bg-black', 'text-gray-light');
            }
        });
        colorLabels.forEach((colorLabel)=>{
            if (colorLabel.textContent === '✔') colorLabel.textContent = '';
    ...
    applyBtn.addEventListener('click', ()=>{
        ...
        activeBorder = 'border-' + modalBg.slice(3);
        activeBg = modalBg;
        modeBtns.forEach((btn)=>{
            if (btn.classList.contains(activeBg) && btn.id === mode) {
                activeTime = parseInt(input.value) * 60000;
                assignTime(activeTime);
            } else if (btn.classList.contains(oldActiveBgBeforeApply) && !btn.classList.contains(activeBg)) {
        ...
        bodyFonts.forEach((font)=>{
            modalFont === font ? body.classList.add(font) : body.classList.remove(font);
        });
...
},{}]},["1B2dF","dV6cC"], "dV6cC", "parcelRequire7e41")
